        ...
        window.QAApp = (function() {
            ...
            function downloadExecutionReport() {
                ...
                const reportData = [
                    ...
                    { 'Total Actions Requested': summary.successful + summary.failed },
                    { 'Successful Actions': summary.successful },
                    { 'Failed Actions': summary.failed },
                    {},
                    { 'Section': 'SUCCESSFUL CHANGES' },
                    ...successfulFixes.map(fix => ({
                        'Status': 'SUCCESS',
                        'Assignment': fix.assignment_name,
                        'Issue Fixed': fix.reason,
                        'Action Applied': fix.fix_action ? getFixActionDescription(fix.fix_action) : 'N/A',
                        'Assignment ID': fix.assignment_id
                    })),
                    {},
                    { 'Section': 'FAILED CHANGES' },
                    ...failedFixes.map(fix => ({
                        'Status': 'FAILED',
                        'Assignment': fix.assignment_name || 'Unknown',
                        'Issue': fix.reason || 'N/A',
                        'Failure Reason': fix.failure_reason,
                        'Assignment ID': fix.assignment_id || 'N/A'
                    }))
                ];
                ...
                const filename = 'qa_execution_report_' + currentTaskId + '_' + new Date().toISOString().slice(0,19).replace(/[:\-]/g, '') + '.xlsx';
